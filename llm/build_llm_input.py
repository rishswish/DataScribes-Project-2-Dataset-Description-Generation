...
def build_summary(row: pd.Series) -> Dict[str, Any]:
    ...
    llm_input = {
        ...
        "source": row["source"],
        "title": row["title"],
        "original_description": trim_text(row["original_description"], max_len=2000),
        ...
        "task": (
            "Generate a clear user-focused dataset description. "
            "Explain what the dataset is about, what kind of columns it contains, "
            "and what kinds of analysis it may support."
        ),
    }
...
def main():
    input_path = Path("data/profiles/nyc_open_data_profiles.parquet")
    output_path = Path("data/descriptions/nyc_open_data_llm_inputs.parquet")
