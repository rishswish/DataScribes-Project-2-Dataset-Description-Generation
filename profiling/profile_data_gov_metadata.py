...
def build_profile_record(row: pd.Series) -> Dict[str, Any]:
    ...
    if not column_names and sample_profile["sample_columns_profile"]:
        column_names = list(sample_profile["sample_columns_profile"].keys())
...
def main():
    input_path = Path("data/metadata/data_gov_metadata_with_samples.parquet")
    output_path = Path("data/profiles/data_gov_profiles.parquet")
